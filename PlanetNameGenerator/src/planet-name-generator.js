...
function loadNames() {
    ...
    for (let i = 0; i < storedNames.length; i++) {
        ...
        savedName.onclick = function() { navigator.clipboard.writeText(savedName.innerHTML) }
        deleteButton.innerHTML = '<i class="fa-regular fa-circle-xmark"></i>'
        deleteButton.onclick = function() { deleteSavedName(savedName.innerHTML) }
...
GENERATOR_BTN.addEventListener("click", () => {
    ...
    for (let i = 0; i < numNames; i++) {
        ...
        name.onclick = function() { navigator.clipboard.writeText(name.innerHTML) }
